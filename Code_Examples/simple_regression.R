...
# Can we predict whether a car is 
...
# Try "logistic" regression glm instead of lm
...
summary(mod3)
exp(coefficients(mod3)) # This gives log-odds: for every 1-unit increase in hp, log-odds of being V-engine drop 0.0685
...
ggplot(df, aes(x=hp,y=vs)) +
  ...
  geom_point(data = df %>% filter(vs != round(binom.pred)),
             color = "red", size = 2)
#binom.pred is this the probability of being 1 or v engine
